...
class TravelWalletRepository implements AbstractCurrencyRepository {
  TravelWalletRepository({required this.dio, required this.travelWalletBox});
  final Dio dio;
  ...
  Future<List<TravelWallet>> getCurrencyRate() async {
    try {
      ...
      wallets.sort((a, b) => a.abbreviation.compareTo(b.abbreviation));

      await travelWalletBox.putAll({
        for (final wallet in wallets) wallet.abbreviation: wallet,
      });
      return wallets;
    } catch (e, st) {
